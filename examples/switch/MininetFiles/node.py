...
class FluidSwitch( Switch ):
    ...
    def start( self, controllers ):
        ...
        self.cmd( 'ifconfig lo up' )
        intfs = [ str( i ) for i in self.intfList() if not i.IP() ]        
        print  'switch -i ' + ' '.join( intfs ) + ' 1> ' + ofdlog + ' 2> ' + ofdlog + ' &'
        self.cmd( 'switch -i ' + ' '.join( intfs ) + ' -d %s ' % self.dpid +
                  ' 1> ' + ofdlog + ' 2> ' + ofdlog + ' &') 

    def stop( self ):
        "Stop Fluid driver example switch."        
        self.cmd( 'kill %switch' )
        self.deleteIntfs()
